"""Keep the Scalper Agent and Trade Guardian alive for this repo.

Supervises two independent child processes — trade_guardian_agent.py and
scalper_agent.py — restarting either one if it exits, with exponential
backoff capped at MAX_RESTART_DELAY_SECONDS. Each child runs under its own
thread so a Guardian crash and a Scalper crash are handled independently.
