...
class PriorityTests(unittest.TestCase):
    ...
    def test_vp_is_first_in_all_triggers(self):
        self.assertEqual(SATriggerEngine.ALL_TRIGGERS[0],
                         "VP_LIQUIDITY_REACTION")

    def test_pre_existing_relative_order_is_unchanged(self):
        """
        Everything below the new head must keep its position, or a disabled VP
        trigger would not reproduce the baseline on bars where two detectors
        fire.
        """
        self.assertEqual(SATriggerEngine.ALL_TRIGGERS[1:],
                         ("SWEEP_REJECTION", "FVG_FILL", "BOS_RETEST",
                          "JUDAS", "VALUE_AREA_FADE"))

    def test_vp_wins_and_records_what_it_outranked(self):
        scn = _Scenario(self.LEVEL, bearish=True)
        eng = SATriggerEngine()
...
class TriggerSetTests(unittest.TestCase):
    ...
    def test_disabled_default_set_equals_the_legacy_five(self):
        self.assertEqual(set(resolve_enabled_triggers(None, False)),
                         {"SWEEP_REJECTION", "FVG_FILL", "BOS_RETEST",
                          "JUDAS", "VALUE_AREA_FADE"})
...
class STBMembershipTests(unittest.TestCase):
    ...
    def test_existing_membership_is_unchanged(self):
        ...
        self.assertEqual(
            DP.STB_NEUTRAL_OK_TRIGGERS - {"VP_LIQUIDITY_REACTION"},
            frozenset({"SWEEP_REJECTION", "JUDAS", "FVG_FILL"}))
        self.assertEqual(
            DP.STB_COUNTER_TREND_TRIGGERS - {"VP_LIQUIDITY_REACTION"},
