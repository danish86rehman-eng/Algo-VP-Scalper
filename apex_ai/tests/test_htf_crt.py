...
class CRTTests(unittest.TestCase):
    ...
    def test_priority_after_sweep_before_vp_and_respects_whitelist(self):
        ...
        self.assertEqual(t.trigger_type, "SWEEP_REJECTION")
        self.assertEqual(t.matched_triggers, ["SWEEP_REJECTION", "HTF_CRT_SWEEP", "VP_LIQUIDITY_REACTION"])
...
class CRTIntegrationTests(unittest.TestCase):
    # These tests isolate CRT execution plumbing from operator entry policy.
    @patch.object(DP, "entry_confidence_allowed", return_value=True)
    def test_backtest_emits_crt_trade_with_shared_metadata(self, confidence_gate):
    ...
    @patch.object(DP, "entry_confidence_allowed", return_value=True)
    def test_scan_reaches_execution_for_all_frames_and_directions(self, confidence_gate):
