...
def resolve_enabled_triggers(requested, vplr_enabled: bool) -> List[str]:
    """
...
class SATriggerEngine:
    """
    SA Simplified Trigger Engine.
    Works on M1/M5 OHLCV data only. No HTF dependency.
    """
    ...
    #: detector that fires. VP_LIQUIDITY_REACTION sits at the head because its
    ...
    #: new head moves.
    ALL_TRIGGERS = ("VP_LIQUIDITY_REACTION", "SWEEP_REJECTION", "FVG_FILL",
    ...
    def step2_trigger(self, df_m5: pd.DataFrame, df_m1: pd.DataFrame,
        ...
        # Highest priority. Needs an anchored H4 profile, which the caller
        ...
        if "FVG_FILL" in self.enabled_triggers:
